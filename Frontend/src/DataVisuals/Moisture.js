...
const Moisture = () => {
  ...
  useEffect(() => {
    ...
    if (!data || !data.data || typeof data.data.steam === "undefined") return;

    // Check if the steam value has changed
    if (data.data.steam !== lastSteamValue) {
      // Update the humidity data and time only if a new steam value is received
      setHumidityData((prevHumidityData) => [
        ...prevHumidityData,
        { time, humidity: data.data.steam }, // Append new data point with the current time
      ]);
      ...
      setLastSteamValue(data.data.steam);
    }
